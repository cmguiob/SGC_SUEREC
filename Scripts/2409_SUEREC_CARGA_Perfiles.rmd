...
geodata_perfiles <- geodata |>
  dplyr::select(UCS, AREA = area, PERFIL) |> #seleccionan variables útiles
...
geodata_vars <- inner_join(geodata_componentes, geodata_perfiles, by = c("UCS", "UCS_comps", "geometry")) |>
...
plot(geodata_vars |> filter(UCS_comps == "CBd-1"))
```


Se consolidan los datos analíticos y los datos geoespaciales
