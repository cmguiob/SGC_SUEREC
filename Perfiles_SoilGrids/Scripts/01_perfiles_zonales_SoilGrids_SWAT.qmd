...
Para acelerar el cálculo se sugiere recortar y guardar el archivo tif localmente. Esto se hace si no hay interferencia con GDAL

```{r}
# (Si es necesario) Proyecta el stack a CRS de los UCS
crs_estudio_ucs <- terra::crs(estudio_ucs_vect)
if (terra::crs(stack_suelo) != crs_estudio_ucs) {
  stack_suelo_proj <- terra::project(stack_suelo, crs_estudio_ucs, method = "bilinear")
} else {
  stack_suelo_proj <- stack_suelo
}

# Recorta al área de interés
ext <- terra::ext(estudio_ucs_vect)
stack_suelo_crop <- terra::crop(stack_suelo_proj, ext)

# GUARDA el recorte como .tif único
writeRaster(stack_suelo_crop, "stack_suelo_crop.tif", overwrite=TRUE)

# 4. LEE el .tif guardado — esto ya es solo un archivo físico pequeño
stack_suelo_tif <- rast("stack_suelo_crop.tif")
...
# Transformar UCs a SpatVector. Este se usa para extraer estadísticas por polígono
estudio_ucs_vect <- vect(estudio_ucs_clip)

# 5. Ahora sí, extracción zonal eficiente
tabla_zonal <- terra::extract(stack_suelo_tif, estudio_ucs_vect, fun=mean, na.rm=TRUE)
...
# Pivot a formato largo (excluyendo ID y usando solo soil_id único)
